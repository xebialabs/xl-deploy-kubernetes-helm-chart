###########################################################################################
# temporary hack to import a fully implemented zip library from python >2.6
# if we ever switch to jython > 2.6 we can use the standard library in module zipfile
#
# used by: jython/export/importAndExport.py
###########################################################################################
...
def main(args = None):
    ...
    USAGE=textwrap.dedent("""\
        Usage:
            zipfileext.py -l zipfile.zip        # Show listing of a zipfile
            zipfileext.py -t zipfile.zip        # Test if a zipfile is valid
            zipfileext.py -e zipfile.zip target # Extract zipfile into target dir
            zipfileext.py -c zipfile.zip src ... # Create zipfile from sources
        """)
    if args is None:
        args = sys.argv[1:]
